...
const introductionElement = createElementStyle(
  "div",
  `font-size:1.2em;position:absolute;text-align:center;transition:opacity 2s;width:280px`,
  "Ko'prikni cho'zish uchun bosib turing. Ko'prik uzunligi to'g'ri bo'lishi kerak, aks holda qo'zichoq yiqilib tushadi!"
);
...
function resetGame() {
  ...
  sceneOffset = 0;
  score = 0;
  gameEnded = false; // Yangi o'yin uchun reset
  ...
  switch (status) {
    ...
    case "falling": {
      ...
      if (santaY > maxSantaY) {
        ...
        // O'yin tugadi - natijani Telegram'ga yuborish
        sendScoreToTelegram();
        return;
      }
      break;
...
function drawSanta() {
  ...
  // Qo'zichoq ranglari
  const brownBody = "#8B4513";  // Jigarrang tana
  const lightBrown = "#D2691E"; // Och jigarrang
  const cream = "#FFEFD5";      // Qorni
  const pink = "#FFB6C1";       // Pushti (quloq ichi)
  const black = "#000000";      // Ko'zlar

  // Tana (oval)
  ctx.beginPath();
  ctx.fillStyle = brownBody;
  ...
  // Oyoqlar (4 ta)
  ctx.fillStyle = brownBody;
  ctx.fillRect(-10, 6, 4, 8);  // Chap orqa
  ctx.fillRect(6, 6, 4, 8);    // O'ng orqa
  ctx.fillRect(-6, 6, 4, 8);   // Chap old
  ctx.fillRect(2, 6, 4, 8);    // O'ng old
  ...
  ctx.fillStyle = "white";
  ctx.beginPath();
  ctx.arc(-3.5, -14.5, 0.7, 0, Math.PI * 2);
  ctx.fill();
  ctx.beginPath();
  ctx.arc(4.5, -14.5, 0.7, 0, Math.PI * 2);
  ctx.fill();
  ...
  // Chap quloq
  ctx.beginPath();
  ctx.ellipse(-10, -15, 4, 7, -0.4, 0, Math.PI * 2);
  ctx.fill();
  // O'ng quloq
  ctx.beginPath();
  ctx.ellipse(10, -15, 4, 7, 0.4, 0, Math.PI * 2);
  ...
  ctx.fillStyle = pink;
  ctx.beginPath();
  ctx.ellipse(-10, -15, 2, 4, -0.4, 0, Math.PI * 2);
  ctx.fill();
  ctx.beginPath();
  ctx.ellipse(10, -15, 2, 4, 0.4, 0, Math.PI * 2);
  ctx.fill();
